...
export default function PlanDetail() {
  ...
  const isNew = urlPlanId === 'new';
  const planId = isNew ? null : urlPlanId;
  ...
  // Buffer for exercises added before the plan is saved (new plan only)
  const [localExerciseIds, setLocalExerciseIds] = useState<string[]>([]);
  ...
  // Auto-save on blur for existing plans only
  ...
  // Save button handler — creates plan + exercises together for new plans
  const handleSave = () => {
    if (!name.trim()) {
      setNameShake(true);
    ...
    if (isNew && localExerciseIds.length > 0) {
      localExerciseIds.forEach(exId => addExerciseToPlan(id, exId));
      setLocalExerciseIds([]);
    }
    if (!currentPlanId) {
  ...
  // Combined set of already-added exerciseIds for visual feedback in picker
  const addedExerciseIds = new Set([
    ...localExerciseIds,
    ...(livePlan?.exercises.map(e => e.exerciseId) ?? []),
  ]);

  const handleAddExercise = (exerciseId: string) => {
    if (isNew) {
      // Buffer locally — dialog stays open for multi-add
      if (!localExerciseIds.includes(exerciseId)) {
        setLocalExerciseIds(prev => [...prev, exerciseId]);
      }
    } else if (currentPlanId) {
      addExerciseToPlan(currentPlanId, exerciseId);
    }
    // Intentionally NOT closing the picker — user can add multiple exercises
  };
  ...
  const totalExerciseCount = isNew ? localExerciseIds.length : currentExercises.length;
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="pt-20 pb-32 px-6 max-w-2xl mx-auto space-y-8">
          ...
          <div className="space-y-4">
            <div className="relative">
              <label
                ...
                style={{ fontFamily: 'Manrope, sans-serif', color: nameShake ? '#95aaff' : 'rgba(149, 170, 255, 0.6)' }}
              >
              ...
              <input
                ref={nameInputRef}
                autoFocus={isNew}
                className="w-full bg-transparent p-0 pt-5 pb-2 text-5xl font-bold text-on-surface focus:outline-none transition-all"
                style={{
                  fontFamily: 'Space Grotesk, sans-serif',
                  animation: nameShake ? 'shake 0.35s ease' : undefined,
                  borderBottom: nameShake
                    ? '2px solid #95aaff'
                    : '2px solid rgba(149, 170, 255, 0.25)',
                }}
              ...
              {TAGS.map(tag => (
                <button
                  ...
                  onClick={() => {
                    ...
                    if (!isNew) setTimeout(handleSaveMeta, 0);
                  }}
        ...
        <section className="space-y-4">
          <div className="flex items-baseline justify-between">
            ...
            <span className="text-xs font-bold text-on-surface-variant tracking-widest uppercase">
              {totalExerciseCount} Gesamt
            </span>
          </div>

          {/* New plan: buffered exercises (simplified — no sets yet) */}
          {isNew && localExerciseIds.length === 0 && (
          ...
          {isNew && localExerciseIds.map((exId, idx) => {
            ...
            return (
              ...
              >
                <div>
                  <span className="text-3xl font-black italic opacity-20 absolute top-2 right-4 pointer-events-none select-none" style={{ fontFamily: 'Space Grotesk, sans-serif' }}>
                    {String(idx + 1).padStart(2, '0')}
                  </span>
                  <h3 className="text-lg font-bold tracking-tight" style={{ fontFamily: 'Space Grotesk, sans-serif' }}>
                    {ex.name}
                  </h3>
                  <p className="text-xs text-on-surface-variant mt-0.5">{ex.category} · {ex.muscleGroups.slice(0, 2).join(', ')}</p>
                </div>
                <button
                  onClick={() => setLocalExerciseIds(prev => prev.filter(id => id !== exId))}
          ...
          {!isNew && currentExercises.length === 0 && (
            <div className="text-center py-10 border-2 border-dashed rounded-xl" style={{ borderColor: 'rgba(72, 72, 71, 0.3)' }}>
              <p className="text-on-surface-variant text-sm">Noch keine Übungen hinzugefügt</p>
            </div>
          )}

          {!isNew && currentExercises.map((pe, idx) => {
          ...
          <button
            ...
            className="w-full py-6 rounded-xl flex items-center justify-center gap-2 transition-all group hover:border-primary cursor-pointer"
        ...
        <div>
          ...
          {!name.trim() && (
            <p className="text-center text-xs text-primary font-bold tracking-widest uppercase mt-2" style={{ opacity: 0.7 }}>
            ...
            <div className="px-6 pt-6 pb-4 flex items-center justify-between">
              <div>
                ...
                {addedExerciseIds.size > 0 && (
                  <p className="text-xs font-bold mt-0.5" style={{ color: '#95aaff', fontFamily: 'Manrope, sans-serif' }}>
                    {addedExerciseIds.size} {addedExerciseIds.size === 1 ? 'Übung' : 'Übungen'} hinzugefügt
                  </p>
                )}
              </div>
              ...
              {filteredExercises.map(ex => {
                ...
                return (
                  <button
                    ...
                    className={`w-full p-4 rounded-xl flex items-start justify-between transition-colors text-left ${
                      isAdded
                        ? 'border'
                    ...
                    style={isAdded ? {
                      ...
                      borderColor: 'rgba(149, 170, 255, 0.3)',
                    } : undefined}
                  >
                    ...
                    <div className="flex flex-col items-end gap-2 shrink-0">
                      ...
                      <span
                        ...
                        style={{
                          color: isAdded ? '#95aaff' : undefined,
                          fontVariationSettings: isAdded ? "'FILL' 1" : undefined,
                        }}
                      >
