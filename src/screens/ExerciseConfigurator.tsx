...
export default function ExerciseConfigurator() {
    ...
    const {plans, updatePlanExerciseSets} = usePlanContext();
                ...
                <section className="space-y-4">
                    <div className="flex items-baseline justify-between mb-3">
                        <h2 className="text-3xl font-bold tracking-tighter"
                            style={{fontFamily: 'Space Grotesk, sans-serif'}}>SÄTZE</h2>
                        <span
                            className="text-xs font-bold text-on-surface-variant tracking-widest uppercase">Ziel: {sets.length} Sätze</span>
                    </div>
                  ...
                  {String(idx + 1).padStart(2, '0')}
                            ...
                            <div className="grid grid-cols-3 gap-4">
                                ...
                                <div className="relative flex flex-col">
                                    <label
                                        className="absolute -top-1 left-0 text-xs font-extrabold text-on-surface-variant tracking-widest uppercase"
                    ...
                    >
                        <span
                            className="material-symbols-outlined text-on-surface-variant group-hover:text-primary transition-colors">add_circle</span>
                        <span
                            className="text-sm font-bold tracking-widest uppercase text-on-surface-variant group-hover:text-primary transition-colors"
                            style={{fontFamily: 'Space Grotesk, sans-serif'}}>
              Satz hinzufügen
            </span>
                    </button>
                ...
                {lastBestSet && (
                    <section className="mt-10 p-6 rounded-xl" style={{
                        ...
                        border: '1px solid rgba(149, 170, 255, 0.1)'
                    }}>
                        <div className="flex items-start gap-4">
                            <div
                                className="w-12 h-12 rounded-xl flex items-center justify-center shrink-0 bg-primary-container">
                            ...
                            <div>
                                ...
                                <p className="text-sm text-on-surface-variant leading-relaxed mt-1">
                                    Bestes Set: <span
                                    className="text-on-surface font-bold">{lastBestSet.weight} kg × {lastBestSet.reps} Wdh.</span>
                                    {lastSession && (
                                        <> — {new Date(lastSession.startedAt).toLocaleDateString('de-DE', {
                                            day: 'numeric',
                                            month: 'short'
                                        })}</>
                                    )}
                ...
                <div className="mt-10">
                    <button
                        ...
                        style={{
                            background: 'linear-gradient(135deg, #95aaff 0%, #3766ff 100%)',
                            boxShadow: '0 4px 20px -5px rgba(149, 170, 255, 0.3)',
